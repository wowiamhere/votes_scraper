...
class GetTheVotes(scrapy.Spider):
    ...
    def _parse_links(self, data_results, congress_session=None):
        for result in data_results:
            ...
            yield scrapy.Request(f'http://www.{self.allowed_domains}{result_url}', callback=self._get_page_data, cb_kwargs=dict(congress_session=congress_session))
    ...
    def _get_page_data(self, response, congress_session=None):
        ...
        temp_string = response.xpath('//div[@id="breadcrumbs"]/div/ol/li[@class="active"]/text()[1]')
        item['congress_session'] = congress_session
        item['senate_or_house'] = temp_string[0]

        reg_exp_search = re.compile('#\d{1,5}\s')
        item['vote_number'] = reg_exp_search.search(temp_string).group()
        ...
        item['percent_yea_votes'] = response.xpath('//table[@class="stats"]/tbody/tr[1]/td[1]/text()').get().strip() + '%'
        item['total_yea_votes'] = response.xpath('//table[@class="stats"]/tbody/tr[1]/td[2]/div[last()]/text()').get().strip()
        item['total_democrat_yeas'] = response.xpath('//table[@class="stats"]/tbody/tr[1]/td[3]/div/text()').get().strip()
        item['total_republican_yeas'] = response.xpath('//table[@class="stats"]/tbody/tr[1]/td[4]/div/text()').get().strip()

        item['percent_nay_votes'] = response.xpath('//table[@class="stats"]/tbody/tr[2]/td[1]/text()').get().strip() + '%'
        item['total_nay_votes'] = response.xpath('//table[@class="stats"]/tbody/tr[2]/td[2]/div[last()]/text()').get().strip()
        item['total_democrat_nays'] = response.xpath('//table[@class="stats"]/tbody/tr[2]/td[3]/div/text()').get().strip()
        item['total_republican_nays'] = response.xpath('//table[@class="stats"]/tbody/tr[2]/td[4]/div/text()').get().strip()

        item['total_votes'] = item['total_yea_votes'] + item['total_nay_votes']
